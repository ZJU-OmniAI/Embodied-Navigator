...
class GRPOConfig(TrainingArguments):
    ...
    Parameters:
        ...
        step_advantage_coef (`float`, *optional*, defaults to `1.0`):
            Coefficient applied to step-level advantages when combining them with trajectory-level advantages during
            rollout training.
        traj_advantage_coef (`float`, *optional*, defaults to `1.0`):
            Coefficient applied to trajectory-level advantages when combining them with step-level advantages during
            rollout training.
    ...
    step_advantage_coef: float = field(
        default=1.0,
        metadata={
            "help": "Coefficient applied to step-level advantages when combining them with trajectory-level "
            "advantages during rollout training."
        },
    )
    traj_advantage_coef: float = field(
        default=1.0,
        metadata={
            "help": "Coefficient applied to trajectory-level advantages when combining them with step-level "
            "advantages during rollout training."
        },
    )
    ...
    # VLN-specific rollout settings
    env_config: str = field(
        default="config/ht_dthink_r2r.yaml",
        metadata={"help": "Habitat environment config used to build DThinkEnv for GRPO rollouts."},
    )
    env_type: str = field(
        default="dthink_base",
        metadata={"help": "Environment registry name passed to env_registry.create."},
    )
    rollout_max_steps: int = field(
        default=64,
        metadata={"help": "Maximum number of interaction steps per trajectory during rollout."},
    )
    step_num_samples: int = field(
        default=1,
        metadata={"help": "Number of candidate actions to sample per environment step during rollout."},
    )
