...
def process_vision_info(
    ...
    return_image_pooling: bool = False,
) -> Tuple[
    ...
    for vision_info in vision_infos:
        if "image" in vision_info or "image_url" in vision_info:
            ...
            image_poolings.append(vision_info.get("pooling", None))
    ...
    if not sum([i is not None for i in image_poolings]):
        image_poolings = None
    else:
        image_poolings = [i if i is not None else -1 for i in image_poolings]

    if return_video_kwargs and return_image_pooling:
        return image_inputs, video_inputs, video_kwargs, image_poolings
    if return_video_kwargs:
        return image_inputs, video_inputs, video_kwargs
    if return_image_pooling:
        return image_inputs, video_inputs, image_poolings
